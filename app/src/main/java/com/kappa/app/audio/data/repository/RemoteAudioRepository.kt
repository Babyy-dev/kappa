...
class RemoteAudioRepository @Inject constructor(
    ...
    override suspend fun approveSeatRequest(roomId: String, seat: Int): Result<Unit> {
        return try {
            val response = apiService.approveSeatRequest(roomId, seat)
            if (!response.success) {
                Result.failure(Exception(response.error ?: "Failed to approve seat"))
            } else {
                Result.success(Unit)
            }
        } catch (throwable: Throwable) {
    ...
    override suspend fun rejectSeatRequest(roomId: String, seat: Int): Result<Unit> {
        return try {
            val response = apiService.rejectSeatRequest(roomId, seat)
            if (!response.success) {
                Result.failure(Exception(response.error ?: "Failed to reject seat"))
            } else {
                Result.success(Unit)
            }
        } catch (throwable: Throwable) {
