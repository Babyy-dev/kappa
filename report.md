...
- Added real posts feed support on the home/postagens tab (backend + client).
- Posts list now pulls from the backend; fallback to sample content only if API fails or returns empty.
- Added follower list adapter and UI; followers now load dynamically from the backend friends API.
- Follower items are clickable and show details in a dialog.
- Rooms header spacing/padding adjusted to improve alignment and visual balance.
