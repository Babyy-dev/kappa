...
class RoomInteractionService(
        ...
        return transaction {
            ...
            rows.asReversed().map { row ->
                ...
                RoomMessageResponse(
                    ...
                    type = row[RoomMessages.type]
                )
            }
        }
    }

    fun sendMessage(roomId: UUID, userId: UUID, message: String, type: String?): MessageResult {
        val trimmed = message.trim()
        if (trimmed.isBlank() || trimmed.length > 500) {
            return MessageResult(failure = MessageFailure.INVALID_MESSAGE)
        }
        val normalizedType = type?.trim()?.uppercase()?.ifBlank { "CHAT" } ?: "CHAT"
        ...
        val result = transaction {
            ...
            RoomMessages.insert {
                ...
                it[RoomMessages.type] = normalizedType
                it[createdAt] = now
            }
        ...
        if (result.response != null && !roomName.isNullOrBlank()) {
            emitRoomEvent(roomId, roomName!!, "messages_changed")
        }
        return result
    }
    ...
    fun sendGift(
        ...
        val result = transaction {
            ...
            val recipients = when (normalizedTarget) {
                "SELF" -> listOf(senderId)
                "ALL" -> {
                    val provided = recipientIds?.toSet().orEmpty()
                ...
                "SELECTED" -> recipientIds?.toSet().orEmpty()
                else -> emptySet()
            }.let { targetIds ->
                if (targetIds.isNotEmpty()) {
                    val seated = RoomParticipants.select {
                        (RoomParticipants.roomId eq roomId) and
                            (RoomParticipants.leftAt.isNull()) and
                            RoomParticipants.seatNumber.isNotNull() and
                            (RoomParticipants.userId inList targetIds.toList())
                    }.map { it[RoomParticipants.userId] }.toSet()
                    if (normalizedTarget == "SELF") {
                        seated.ifEmpty { setOf(senderId) }
                    } else {
                        seated
                    }
                } else {
                    if (giftType == "INDIVIDUAL") {
                        ...
                        setOf(recipientRow[RoomParticipants.userId])
                    } else {
                        RoomParticipants.select {
                            (RoomParticipants.roomId eq roomId) and
                                (RoomParticipants.leftAt.isNull()) and
        ...
        if (result.response != null && !roomName.isNullOrBlank()) {
            emitRoomEvent(roomId, roomName!!, "gifts_changed")
        }
        return result
    }
    ...
    private fun emitRoomEvent(roomId: UUID, roomName: String, type: String) {
