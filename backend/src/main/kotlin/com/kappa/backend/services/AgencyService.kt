...
class AgencyService(
    ...
    fun listAgencyRooms(agencyId: UUID): List<AgencyRoomResponse> {
        return transaction {
            Rooms.select { Rooms.agencyId eq agencyId }
    ...
    fun listAgencyHosts(agencyId: UUID): List<AgencyHostResponse> {
        return transaction {
            val hostRows = Users.select {
                (Users.agencyId eq agencyId) and (Users.role eq UserRole.HOST.name)
            }.orderBy(Users.createdAt, SortOrder.DESC)

            hostRows.map { row ->
                val userId = row[Users.id]
                val diamonds = DiamondWallets.select { DiamondWallets.userId eq userId }
                    .singleOrNull()
                    ?.get(DiamondWallets.balance)
                    ?: 0L
                AgencyHostResponse(
                    id = userId.toString(),
