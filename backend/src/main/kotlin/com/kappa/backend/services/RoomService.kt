...
import com.kappa.backend.models.RoomEventEnvelope
import com.kappa.backend.util.CountryRegion
...
class RoomService(
        ...
        return transaction {
            ...
            val agencies = if (agencyIds.isNotEmpty()) {
                Agencies.select { Agencies.id inList agencyIds }.associate { row ->
                    row[Agencies.id] to Pair(row[Agencies.name], row[Agencies.iconUrl])
                }
            } else {
                emptyMap()
            }
            rows.map { row ->
                    ...
                    val agencyInfo = row[Rooms.agencyId]?.let { agencies[it] }
                    val agencyName = agencyInfo?.first
                    val agencyIcon = agencyInfo?.second
                    val country = row[Rooms.country]
                    RoomResponse(
                        id = roomId.toString(),
                        name = row[Rooms.name],
    ...
    fun joinRoom(roomId: UUID, userId: UUID, username: String, password: String?): JoinRoomResult {
        return transaction {
            ...
            if (roomRow[Rooms.status] != "active") {
                return@transaction JoinRoomResult(failure = JoinRoomFailure.ROOM_NOT_FOUND)
            }
    ...
    fun takeSeat(roomId: UUID, seatNumber: Int, userId: UUID): String? {
        val error = transaction {
            ...
            if (seatStatus == SeatStatus.REQUESTED && seatRow[RoomSeats.userId] != userId) {
                return@transaction "Seat already requested"
            }
    ...
    fun requestSeat(roomId: UUID, seatNumber: Int, userId: UUID): String? {
        val error = transaction {
            val roomRow = Rooms.select { Rooms.id eq roomId }.singleOrNull()
                ?: return@transaction "Room not found"
            val maxSeats = roomRow[Rooms.maxSeats] ?: 28
            ...
            val seatStatus = SeatStatus.valueOf(seatRow[RoomSeats.status])
            val occupiedBy = seatRow[RoomSeats.userId]
            if (seatStatus == SeatStatus.OCCUPIED && occupiedBy != userId) {
                return@transaction "Seat already occupied"
            }
            if (seatStatus == SeatStatus.REQUESTED && occupiedBy != null && occupiedBy != userId) {
                return@transaction "Seat already requested"
            }

            if (seatMode != SeatMode.BLOCKED && seatStatus != SeatStatus.BLOCKED) {
                return@transaction "Seat does not require request"
            }
            ...
            RoomSeats.update({ (RoomSeats.roomId eq roomId) and (RoomSeats.seatNumber eq seatNumber) }) {
                it[RoomSeats.userId] = userId
                it[RoomSeats.status] = SeatStatus.REQUESTED.name
            }
            RoomParticipants.update({ RoomParticipants.id eq participant[RoomParticipants.id] }) {
                it[RoomParticipants.role] = ParticipantRole.LISTENER.name
                it[RoomParticipants.seatNumber] = null
    ...
    fun approveSeatRequest(roomId: UUID, seatNumber: Int): String? {
        val error = transaction {
            ...
            val seatStatus = SeatStatus.valueOf(seatRow[RoomSeats.status])
            if (seatStatus != SeatStatus.REQUESTED) {
                return@transaction "Seat has no pending request"
            }
            val requester = seatRow[RoomSeats.userId] ?: return@transaction "Seat has no requester"
            val participant = RoomParticipants.select {
                (RoomParticipants.roomId eq roomId) and
                    (RoomParticipants.userId eq requester) and
                    RoomParticipants.leftAt.isNull()
            }.singleOrNull() ?: return@transaction "Requester not in room"

            clearSeatForUser(roomId, requester)

            RoomSeats.update({ (RoomSeats.roomId eq roomId) and (RoomSeats.seatNumber eq seatNumber) }) {
                it[RoomSeats.userId] = requester
                it[RoomSeats.status] = SeatStatus.OCCUPIED.name
            }
            RoomParticipants.update({ RoomParticipants.id eq participant[RoomParticipants.id] }) {
                it[RoomParticipants.role] = ParticipantRole.SPEAKER.name
    ...
    fun unlockSeat(roomId: UUID, seatNumber: Int): String? {
        val error = transaction {
            ...
            val seatStatus = SeatStatus.valueOf(seatRow[RoomSeats.status])
            if (seatStatus != SeatStatus.BLOCKED && seatStatus != SeatStatus.REQUESTED) {
    ...
    fun banParticipant(roomId: UUID, targetUserId: UUID): String? {
        ...
        return kickParticipant(roomId, targetUserId)
    }
    ...
    private fun emitSeatsChanged(roomId: UUID) {
        emitRoomEvent(roomId, "seats_changed")
    }

    private fun emitRoomEvent(roomId: UUID, type: String) {
        val roomName = transaction {
            Rooms.select { Rooms.id eq roomId }.singleOrNull()?.get(Rooms.name)
        } ?: return
        val payload = json.encodeToString(RoomEventEnvelope(type = type, roomId = roomId.toString()))
        liveKitRoomService.sendData(roomName, payload.toByteArray(Charsets.UTF_8), topic = "room_events")
    }
}
