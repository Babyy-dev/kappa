...
object RoomMessages : Table("room_messages") {
    ...
    val userId = uuid("user_id").index()
    val message = varchar("message", 500)
    val type = varchar("type", 16).default("CHAT")
    val createdAt = long("created_at")
    override val primaryKey = PrimaryKey(id)
}
...
object AdminWithdrawalRules : Table("admin_withdrawal_rules") {
    val id = uuid("id")
    val minAmountUsd = double("min_amount_usd")
    val multipleUsd = double("multiple_usd")
    val updatedAt = long("updated_at")
    override val primaryKey = PrimaryKey(id)
}
