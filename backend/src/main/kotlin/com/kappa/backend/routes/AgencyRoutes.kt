...
import com.kappa.backend.models.AgencyHostResponse
import com.kappa.backend.models.AgencyRoomResponse
...
fun Route.agencyRoutes(agencyService: AgencyService) {
    get("agency/applications") {
        ...
        val response = if (role == "ADMIN") {
            agencyService.listAgencyApplications()
        } else {
            agencyService.listAgencyApplicationsForUser(UUID.fromString(userId))
        }
    ...
    get("agency/rooms") {
        ...
        if (agencyId == null) {
            call.respond(ApiResponse(success = true, data = emptyList<AgencyRoomResponse>()))
            return@get
        }
        val response = agencyService.listAgencyRooms(agencyId)
        call.respond(ApiResponse(success = true, data = response))
    }
    ...
    get("agency/hosts") {
        ...
        if (agencyId == null) {
            call.respond(ApiResponse(success = true, data = emptyList<AgencyHostResponse>()))
            return@get
        }
        val response = agencyService.listAgencyHosts(agencyId)
        call.respond(ApiResponse(success = true, data = response))
    }
