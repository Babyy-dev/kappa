...
import com.kappa.backend.data.AdminWithdrawalRules as AdminWithdrawalRulesTable
...
import com.kappa.backend.models.AdminWithdrawalRules
...
fun Route.adminRoutes() {
    ...
    get("admin/config/withdrawals") {
        if (requireAdmin(call) == null) {
            call.respond(HttpStatusCode.Forbidden, ApiResponse<Unit>(success = false, error = "Insufficient permissions"))
            return@get
        }
        val rules = transaction {
            AdminWithdrawalRulesTable.selectAll().singleOrNull()?.let { row ->
                AdminWithdrawalRules(
                    minAmountUsd = row[AdminWithdrawalRulesTable.minAmountUsd],
                    multipleUsd = row[AdminWithdrawalRulesTable.multipleUsd]
                )
            }
        }
        call.respond(ApiResponse(success = true, data = rules))
    }

    post("admin/config/withdrawals") {
        val actorId = requireAdmin(call) ?: return@post call.respond(
            HttpStatusCode.Forbidden,
            ApiResponse<Unit>(success = false, error = "Insufficient permissions")
        )
        val request = call.receive<AdminWithdrawalRules>()
        transaction {
            val now = System.currentTimeMillis()
            val existing = AdminWithdrawalRulesTable.selectAll().singleOrNull()
            if (existing == null) {
                AdminWithdrawalRulesTable.insert {
                    it[id] = UUID.randomUUID()
                    it[minAmountUsd] = request.minAmountUsd
                    it[multipleUsd] = request.multipleUsd
                    it[updatedAt] = now
                }
            } else {
                AdminWithdrawalRulesTable.update({ AdminWithdrawalRulesTable.id eq existing[AdminWithdrawalRulesTable.id] }) {
                    it[minAmountUsd] = request.minAmountUsd
                    it[multipleUsd] = request.multipleUsd
                    it[updatedAt] = now
                }
            ...
            AdminAuditLogs.insert {
                it[AdminAuditLogs.id] = UUID.randomUUID()
                it[AdminAuditLogs.actorId] = UUID.fromString(actorId)
                it[AdminAuditLogs.action] = "WITHDRAW_RULES_UPDATE"
                it[AdminAuditLogs.message] = "Withdrawal rules updated"
                it[AdminAuditLogs.createdAt] = now
            }
        }
        call.respond(ApiResponse(success = true, data = request))
