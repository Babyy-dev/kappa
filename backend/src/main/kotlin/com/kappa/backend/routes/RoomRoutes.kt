...
fun Route.roomRoutes(
    ...
    post("rooms/{id}/seats/{seat}/request") {
        ...
        val error = roomService.requestSeat(UUID.fromString(roomId), seatNumber, UUID.fromString(userId))
        if (error != null) {
            call.respond(HttpStatusCode.BadRequest, ApiResponse<Unit>(success = false, error = error))
            return@post
        }
        call.respond(ApiResponse(success = true, data = Unit, message = "Seat requested"))
    }
    ...
    post("rooms/{id}/seats/{seat}/approve") {
        ...
        val error = roomService.approveSeatRequest(UUID.fromString(roomId), seatNumber)
        if (error != null) {
            call.respond(HttpStatusCode.BadRequest, ApiResponse<Unit>(success = false, error = error))
            return@post
        }
        call.respond(ApiResponse(success = true, data = Unit, message = "Seat approved"))
    }
    ...
    post("rooms/{id}/seats/{seat}/reject") {
        ...
        val error = roomService.rejectSeatRequest(UUID.fromString(roomId), seatNumber)
        if (error != null) {
            call.respond(HttpStatusCode.BadRequest, ApiResponse<Unit>(success = false, error = error))
            return@post
        }
        call.respond(ApiResponse(success = true, data = Unit, message = "Seat rejected"))
    }
    ...
    post("rooms/{id}/gifts") {
        ...
        val result = roomInteractionService.sendGift(
            ...
            recipientIds = request.recipientIds?.mapNotNull { value -> runCatching { UUID.fromString(value) }.getOrNull() }
        )
        if (result.response == null) {
            val (status, message) = when (result.failure) {
